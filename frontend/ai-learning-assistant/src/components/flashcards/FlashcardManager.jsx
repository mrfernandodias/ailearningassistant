...
const FlashcardManager = ({ documentId }) => {
  ...
  const fetchFlashcardSets = async () => {
    ...
    try {
      ...
      const raw = payload?.data ?? payload?.sets ?? payload;
  ...
  const handleReview = async (index) => {
    ...
    try {
      ...
      console.error('Error to review flashcard', error);
    }
  };
  ...
  const handleToggleStar = async (cardId) => {
    try {
      ...
      const selId = selectedSet?._id ?? selectedSet?.id;
      const newSelected = updatedSets.find(
        (set) => (set._id ?? set.id) === selId,
      );
      setSelectedSet(newSelected || selectedSet);
      toast.success('Flashcard starred status updated!');
    } catch (error) {
      toast.error('Failed to update star status');
      console.error('Error to star flashcard:', error);
    }
  };
          ...
          {flashcardSets.map((set) => (
            ...
            >
              ...
              <div className="space-y-4">
                <div className="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-gradient-to-br from-emerald-100 to-emerald-100">
